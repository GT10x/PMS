...
export default function ProjectModulesPage() {
  ...
  const updateModuleOverview = async (moduleId: string, overview: string) => {
    setSavingOverview(true);
    try {
      const res = await fetch(`/api/projects/${projectId}/modules`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ module_id: moduleId, overview: overview || null }),
      });
      if (res.ok) {
        const { module: updated } = await res.json();
        setModules(prev => prev.map(m => m.id === moduleId ? { ...m, overview: updated.overview } : m));
              ...
              >
                ...
                {isExpanded && (
                  <div className="px-4 pb-4 border-t border-gray-100 dark:border-gray-700">
                    ...
                    {canManageModules() ? (
                      <div className="mt-4">
                        <h4 className="text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Module Overview:</h4>
                        {editingOverview === module.id ? (
                          <div className="space-y-2">
                            <textarea
                              value={overviewText}
                              onChange={(e) => setOverviewText(e.target.value)}
                              rows={4}
                              className="w-full px-3 py-2 text-sm border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-indigo-500 resize-y"
                              placeholder="Add module motive, objectives, scope..."
                              autoFocus
                            />
                            <div className="flex gap-2">
                              <button
                                onClick={() => updateModuleOverview(module.id, overviewText)}
                              ...
                              <button
                                ...
                                className="px-3 py-1 text-xs font-medium bg-gray-200 dark:bg-gray-600 text-gray-700 dark:text-gray-300 rounded hover:bg-gray-300 dark:hover:bg-gray-500"
                              >
                                Cancel
                              </button>
                            </div>
                          ...
                          <div
                            onClick={() => { setEditingOverview(module.id); setOverviewText(module.overview || ''); }}
                            className="text-sm text-gray-600 dark:text-gray-400 cursor-pointer hover:bg-gray-100 dark:hover:bg-gray-700 rounded-lg px-3 py-2 transition-colors whitespace-pre-wrap"
                          >
                            {module.overview || <span className="italic text-gray-400 dark:text-gray-500">Click to add module overview...</span>}
                          </div>
                        )}
                      </div>
                    ) : (
                      module.overview && (
                        <div className="mt-4">
                          <h4 className="text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Module Overview:</h4>
                          <p className="text-sm text-gray-600 dark:text-gray-400 px-3 py-2 whitespace-pre-wrap">{module.overview}</p>
                        </div>
                      )
                    )}
